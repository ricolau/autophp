...
//    $v = $md->set('counter', null,1, $token);
    ...
    public function set($key, $val, $expire) {
        ...
        ($timeCost = microtime(true) - $_debugMicrotime) && performance::add(__METHOD__, $timeCost, array('alias'=>$this->_alias,'key'=>$key,'val'=>$val ,'retType'=>(!$ret ? $ret: gettype($ret))       )) && auto::isDebug() && auto::debugMsg(__METHOD__, 'cost ' . $timeCost . 's, arguments: ' . var_export(array($key,$val,$expire), true));
