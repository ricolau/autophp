...
class cache_redis extends cache_abstract{
    ...
    public function __construct($alias, $conf){
        ...
        if(!isset($this->_confs['connectTimeout'])){
            $this->_confs['connectTimeout'] = self::connect_timeout_default;
        }
    ...
    public function connect(){
        ...
        if(!$this->_confs['host'] || !$this->_confs['port']){
            throw new exception_cache(
            'redis connection host and port error!' . (auto::isDebug() ? var_export($this->_confs, true) : ''), exception_cache::type_server_connection_error
            );
        }
        try{
